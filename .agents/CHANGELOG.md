...
Run the controlled Windows peer group and the `demo/run.ps1` topology check
...
| Windows controlled-peer RPC integration | 27 passed with a real peer host, web server, and event-listener adapter |
...
- real Supervisor/RPC integration and cross-runtime central-to-peer lanes where
  a separately managed peer environment is available;
...
#### Supervisor peer ownership

Supervisor is a peer runtime for the two RPC adapters. The in-process
`multivisor.rpc` module and the `multivisor-rpc` event listener execute in the
same environment as an existing Supervisor process; the central `multivisor`
web process and `multivisor-cli` do not run Supervisor internally. Public
`rpc`, `web`, and `all` extras therefore do not install, upgrade, constrain, or
document the peer runtime.

The Multivisor package and its `rpc` extra support CPython 3.12–3.14. Peer
runtime selection belongs to the host administrator, and central and peer
processes may use separate machines and Python environments. The private
`rpc-test` group and Docker's explicit demo dependency exist only to create
controlled test topologies; they are not a public peer-support policy.

#### Distribution selection and the same-wheel topology

No separate Multivisor build is required for central and RPC-adapter processes.
Multivisor is pure Python, and its wheel is a universal `py3-none-any`
distribution with `Requires-Python: >=3.12`. Both environments install that
same artifact:
...
| RPC adapter | Python 3.12–3.14 | `multivisor[rpc]` | Existing Supervisor peer |
...
1. `pyproject.toml`
   ...
   - add a private `rpc-test` group with platform-scoped Supervisor test peers;
   - do not override independently managed peer dependencies;
...
3. `.github/workflows/python.yml`
   ...
   - add an RPC matrix across the adapter compatibility lanes using
     `--group rpc-test` and separately managed peer fixtures;
...
12. `README.md`, `PACKAGING.md`, `AGENTS.md`, and `.agents/*`
    - synchronize setup, support policy, peer ownership boundary, and validation.
...
The lock retains platform-scoped peer-test dependencies behind the private
RPC-test marker. Core environments never select those peer dependencies.
...
# Windows: run the controlled peer integration lane
...
| --- | --- |
| Initial Windows all-extras peer resolution | Failed at the external peer boundary; established the need for a runtime split |
| Windows Python 3.14 core after peer isolation, before event-listener test | 3 passed, 19 skipped; skips were Supervisor-dependent tests |
| Windows controlled-peer full suite before event-listener test | 22 passed |
| Windows Python 3.14 central to isolated rpcinterface host | 21 passed, 1 skipped |
| Focused Python 3.14 central to isolated event-listener host | 1 passed |
...
| Windows controlled-peer RPC integration | 24 passed, including both adapters and actual process-event forwarding |
...
| Windows 3.13 central to isolated RPC host | 23 passed, 1 expected host-only entry-point skip |
| Windows 3.14 central to isolated RPC host | 23 passed, 1 expected host-only entry-point skip |
...
- Operational: Python 3.10 and 3.11 installations are no longer supported;
  default development and containers move to standard CPython 3.14. The central
  process and independently managed RPC peer may use separate environments
  while installing the same Multivisor wheel.
...
also leaves the independent peer topology implicit. The former
`examples/full_example/` directory has been flattened to `demo/`: this is one
manual integration demo rather than a family of usage examples. `demo/run.ps1`
makes its topology repeatable.
...
The Windows peer fixture could not locate bare `python` commands in the
original Unix-oriented demo INI files. The launcher therefore writes ignored,
same-directory `*.windows.runtime.conf` copies with the peer executable quoted
in each program command. Keeping the copies beside the originals preserves
Supervisor's `%(here)s` expansion for programs and logs. The source INI files
are unchanged, so Unix usage remains unchanged.
